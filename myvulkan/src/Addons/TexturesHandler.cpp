...
TexturesHandler::TexturesHandler(std::string path) {
	this->path = path;
}
...
TexturesHandler::~TexturesHandler() {
	for (auto imageSampler : imageSamplers) {
		vkDestroySampler(devicesHandler->device, imageSampler, nullptr);
	}
...
void TexturesHandler::addTexture(std::string filename) {
	...
	addImage(filename, &image, &imageMemory);
	imageView = addImageView(image);
	imageSampler = addSampler();
...
void TexturesHandler::addImage(std::string filename, VkImage* image, VkDeviceMemory* imageMemory) {
	...
	stbi_uc* pixels = stbi_load((path + filename).c_str(), &texWidth, &texHeight, &texChannels, STBI_rgb_alpha);
	...
	createImage(texWidth, texHeight, mipLevels, format, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_TRANSFER_SRC_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, *image, *imageMemory);
	...
	generateMipmaps(*image, format, texWidth, texHeight, mipLevels);
...
void TexturesHandler::generateMipmaps(VkImage image, VkFormat imageFormat, int32_t texWidth, int32_t texHeight, uint32_t mipLevels) {
...
VkImageView TexturesHandler::addImageView(VkImage image) {
	return createImageView(image, VK_FORMAT_R8G8B8A8_UNORM, VK_IMAGE_ASPECT_COLOR_BIT, mipLevels);
};


VkSampler TexturesHandler::addSampler() {
	VkSampler sampler;
