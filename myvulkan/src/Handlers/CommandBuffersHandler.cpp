...
void CommandBuffersHandler::internalCreateCommandBuffers(std::vector<VkCommandBuffer>* buffers) {
	...
	for (size_t i = 0; i < (*buffers).size(); i++) {
		...
		for (size_t j = 0; j < modelsHandler->models.size(); j++) {
			...
			vkCmdBindDescriptorSets((*buffers)[i], VK_PIPELINE_BIND_POINT_GRAPHICS, pipelinesHandler->pipelineLayout, 0, 1, &descriptorsHandler->descriptorSets[i], 1, &dynamicOffset);
